...
export async function generateMetadata({
  ...
  const { t } = await initTranslations(locale, ["cookies"]);
...
export default async function CookiesPage({
  ...
  const content = t("cookies:content", {
    returnObjects: true,
  }) as LegalDocumentContent | undefined;
  const translationPending = t("cookies:translation_pending", {
    defaultValue: "",
  });
  ...
  return (
    <TranslationsProvider
      locale={locale}
      resources={resources}
      namespaces={["cookies", "common"]}
    >
      <PageTitle
        title={t("cookies:page_title")}
        breadcrumbs={[
          { label: t("common:nav.home"), href: "/" },
          { label: t("cookies:breadcrumbs.cookies") },
        ]}
      />
